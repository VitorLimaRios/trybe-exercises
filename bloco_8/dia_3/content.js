...
const sum = numbers.reduce((acc, num) => (num % 2 === 0 ? acc + num : acc));

const assert = require('assert');

assert.deepStrictEqual(sum, 152);
...
const studentsHigherGrade = estudantes.map((estudante) => {
  return {
    name: estudante.nome,
    materia: estudante.materias.reduce((acc, aluno) => (acc.nota > aluno.nota ? acc : aluno)).name,
  };
});

assert.deepStrictEqual(studentsHigherGrade, expected);
